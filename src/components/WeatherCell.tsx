...
function WeatherCell({ day, date, low, high, text /* temp, tempMin, tempMax, feelsLike, humidity, windSpeed, description, icon, timestamp */ }: YahooWeatherObject) {
			...
			<Popover width={200} position="bottom" withArrow shadow="md" opened={opened}>
				<Popover.Target>
					<div onMouseEnter={open} onMouseLeave={close} className="flex flex-col items-center space-y-2 bg-gray-100 shadow rounded p-2 my-2 cursor-pointer hover:bg-gray-200">
						<p className="font-light">{`${new Date(date * 1000).toDateString()}`}</p>
				...
				<Popover.Dropdown sx={{ pointerEvents: "none", backgroundColor: "#fafafa", zIndex: "auto" }}>
					<div className="flex flex-col space-y-2 z-50">
						<p className="font-extrabold text-center">{`${new Date(date * 1000).toDateString()}`}</p>
						...
						<div className="flex flex-col">
							<div className="flex justify-between">
								...
								<p className="text-black font-bold">{`${celsius ? `${low.toFixed(0)}°C` : `${celsiusToFahrenheit(low).toFixed(0)}°F`}`}</p>
							</div>
							<div className="flex justify-between">
								<p className="text-gray-500">Max Temp</p>
								<p className="text-black font-bold">{`${celsius ? `${high.toFixed(0)}°C` : `${celsiusToFahrenheit(high).toFixed(0)}°F`}`}</p>
							</div>
						</div>
					</div>
				</Popover.Dropdown>
