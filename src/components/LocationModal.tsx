...
import { RiUserLocationLine, RiPlaneLine, RiTrainFill, RiCarFill, RiCloseLine } from "react-icons/ri";
...
function LocationModal() {
	...
	const { data: cityImages, isLoading } = trpc.useQuery(["places.getPlaceImagesFromCityName", { selectedCityName: selectedCityName }], {
	...
	return (
		<>
			<Modal centered withCloseButton={false} opened={isLocationModalOpen} onClose={() => setIsLocationModalOpen(false)} overlayOpacity={0.55} overlayBlur={3} size="75%" padding={0} overflow="inside" transition="fade" transitionDuration={600} transitionTimingFunction="ease">
				<div>
					{isLoading ? (
						...
						<Carousel
							...
							align="start"
						>
							<Carousel.Slide>{cityImages && <img src={cityImages[0]} className="h-full w-full object-cover" />}</Carousel.Slide>
							<Carousel.Slide>{cityImages && <img src={cityImages[1]} className="h-full w-full object-cover" />}</Carousel.Slide>
							<Carousel.Slide>{cityImages && <img src={cityImages[2]} className="h-full w-full object-cover" />}</Carousel.Slide>
							<Carousel.Slide>{cityImages && <img src={cityImages[3]} className="h-full w-full object-cover" />}</Carousel.Slide>
							<Carousel.Slide>{cityImages && <img src={cityImages[4]} className="h-full w-full object-cover" />}</Carousel.Slide>
							<Carousel.Slide>{cityImages && <img src={cityImages[5]} className="h-full w-full object-cover" />}</Carousel.Slide>
						</Carousel>
					)}
				</div>
				<div className="flex flex-col bg-white p-8">
					<div className="flex items-center justify-between p-4 whitespace-nowrap">
						<div className="w-full flex justify-between items-center">
							<div>
								<h2 className="text-xl font-semibold text-black font-mono">{selectedCityName}</h2>
							...
							<div className="flex space-x-6 text-xl">
								...
								{selectedCityDriveTime != "" && (
									<div className="flex items-center space-x-2">
										<RiCarFill />
										<p className="text-gray-500 text-base">{selectedCityDriveTime}</p>
									</div>
								)}
								<div onClick={() => setIsLocationModalOpen(false)} className={`text-gray-500 bg-gray-100 hover:bg-gray-200 text-lg p-4 cursor-pointer`}>
									<RiCloseLine />
								</div>
							</div>
						</div>
					...
					<div className="flex space-x-4" ref={datepickerWrapperRef}>
						...
						<SearchFormField title="When" icon={<AiOutlineCalendar className="text-4xl text-yellow-500 md:inline-flex cursor-pointer md:mx-2" />}>
							<>
								<div className="flex justify-between items-center font-mono" onClick={() => setIsDatepickerOpen(true)}>
				...
				<div className="flex flex-col space-y-8 bg-gray-50 py-8 px-4">
					<div className="flex space-x-6 font-mono font-bold text-lg">
						<button onClick={() => setSelectedTab("attractions")} className={`p-3 shadow rounded-lg ${selectedTab === "attractions" ? "bg-blue-500 text-white hover:bg-blue-600" : "bg-white text-gray-500 hover:bg-gray-100"}`}>
							THINGS TO DO
						</button>
						<button onClick={() => setSelectedTab("accommodation")} className={`p-3 shadow rounded-lg ${selectedTab === "accommodation" ? "bg-blue-500 text-white hover:bg-blue-600" : "bg-white text-gray-500 hover:bg-gray-100"}`}>
							ACCOMMODATION
						</button>
						<button onClick={() => setSelectedTab("restaurants")} className={`p-3 shadow rounded-lg ${selectedTab === "restaurants" ? "bg-blue-500 text-white hover:bg-blue-600" : "bg-white text-gray-500 hover:bg-gray-100"}`}>
							RESTAURANTS
						</button>
