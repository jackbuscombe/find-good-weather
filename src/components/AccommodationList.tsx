...
function AccommodationList({ lat, long }: Props) {
	const selectedCityLat = useStore((state) => state.selectedCityLat);
	const selectedCityLong = useStore((state) => state.selectedCityLong);

	const { data: accommodations, isLoading } = trpc.useQuery(["accommodation.getHotelsNearCoordinates", { lat, long }], {
		enabled: !!lat && !!long,
		staleTime: Infinity,
		cacheTime: Infinity,
	});
	...
	return (
		<div className="w-full flex flex-col space-y-12">
			...
			{accommodations &&
				...
				accommodations.map(({ name, photo, placeId, rating, userRatingsTotal, vicinity, lat, long }) => (
