...
export const attractionsRouter = createRouter().query(
    ...
    async resolve({ input }) {
      try {
        const response = await axios.get(
          `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${input.lat}%2C${input.long}&radius=1500&type=tourist_attraction&key=${process.env.GOOGLE_MAPS_API}`
        );
        ...
        for (let i = 0; i < results.length; i++) {
          if (results[i].business_status === "OPERATIONAL") {
            ...
            attractionsArray.push({
              ...
              editorialSummary: results?.[i]?.editorial_summary ?? "",
            });
          }
        }
