...
export const accommodationRouter = createRouter().query("getHotelsNearCoordinates", {
	input: z.object({
		lat: z.number(),
		long: z.number(),
	}),
	async resolve({ input }) {
		try {
			const response = await axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${input.lat}%2C${input.long}&radius=1500&type=lodging&key=${process.env.GOOGLE_MAPS_API}`);
